...
export function saveNotificationData(details, onSuccess, onFailure) {
 
  AsyncStorage.setItem("notificationList", JSON.stringify(details)).then(
    success => {
...
export function clearNotificationData(onSuccess, onError) {
...
export function getNotificationList(onSuccess, onCartNotFound, onFailure) {
  AsyncStorage.getItem("notificationList").then(
    response => {
      if (response != "" && response != null && response != undefined) {
